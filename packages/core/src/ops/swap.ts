...
export function getHiveDepositOp(params: {
  ...
  if (amount.asset !== "HIVE" && amount.asset !== "HBD") {
    throw new Error(
      `getHiveDepositOp: asset must be HIVE or HBD, got ${amount.asset}`,
    );
  }
  const chainUnit =
    amount.asset === "HBD"
  ...
  const defaultMemo = new URLSearchParams(
    `to=${toDid.split(":").at(-1) ?? ""}`,
  );
...
export function getHiveSwapOp(params: {
  ...
  const referral =
    params.referralOverride === undefined
      ? config.referral
      : params.referralOverride;
  ...
  const payload: Record<string, string | number> = {
    ...
    min_amount_out:
      finalMinAmountOut !== undefined ? finalMinAmountOut.toString() : "0",
