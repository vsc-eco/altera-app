import type { SwapAsset } from "@magi/core";
...
export function createPoolPriceProvider(pools: PoolProvider): PriceProvider {
  ...
  async function refresh(): Promise<Record<string, number>> {
    ...
    if (hivePool) {
      const hbdR =
        hivePool.asset0 === "hbd" ? hivePool.reserve0 : hivePool.reserve1;
      const hiveR =
        hivePool.asset0 === "hbd" ? hivePool.reserve1 : hivePool.reserve0;
    ...
    if (btcPool) {
      const btcR =
        btcPool.asset0 === "btc" ? btcPool.reserve0 : btcPool.reserve1;
      const hbdR =
        btcPool.asset0 === "btc" ? btcPool.reserve1 : btcPool.reserve0;
      if (btcR > 0n) {
