...
export function MagiQuickSwap(props: MagiQuickSwapProps) {
			...
			try {
				if (isBtcInput) {
					if (assetOut === 'HBD') {
						...
						if (!p1 || !p2) {
							if (!cancelled) setPreviewError('Missing pool for BTC→HIVE route');
							return;
						}
	...
	const recipientValid = useMemo(() => {
		...
		if (assetOut === 'BTC')
			return validateBtcAddr(
				recipient.trim(),
				config.network === 'vsc-testnet' ? BtcNetwork.testnet : BtcNetwork.mainnet
			);
	...
	const inputAmount = useMemo(() => {
		try {
			return amountInStr ? CoinAmount.fromDecimal(amountInStr, assetIn).raw : 0n;
		} catch {
			return 0n;
		}
	}, [amountInStr, assetIn]);
	...
	return (
		<div className={`magi-quickswap ${className ?? ''}`}>
			...
			<div className="magi-qs-receiver">
				...
				{isBtcInput && !recipientValid && (
					<p className="magi-qs-receiver-hint">
						Enter the Hive account that will receive {assetOut}. The deposit address is generated
						from this.
					</p>
				)}
				<div
				...
				<div className="magi-qs-slippage-options">
					{SLIPPAGE_PRESETS.map(({ bps, label }) => (
						<button
							key={bps}
							type="button"
							className={!customSlippageOpen && slippageBps === bps ? 'active' : ''}
							onClick={() => {
								setSlippageBps(bps);
					...
					{customSlippageOpen ? (
							...
							<input
								...
								onChange={(e) => {
									...
									if (Number.isFinite(p))
										setSlippageBps(Math.round(Math.max(0.01, Math.min(99.99, p)) * 100));
								}}
								disabled={submitting}
							/>
							<span>%</span>
						...
						<button
							type="button"
							className={!SLIPPAGE_PRESETS.some((p) => p.bps === slippageBps) ? 'active' : ''}
							onClick={() => {
			...
			{exceedsBalance && (
				<p className="magi-qs-status error">Amount exceeds your wallet balance.</p>
			)}
