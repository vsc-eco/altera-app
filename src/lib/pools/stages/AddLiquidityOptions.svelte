<script lang="ts">
	...
	let amount0Ca = $state(new CoinAmount(0, Coin.hive));
	let amount1Ca = $state(new CoinAmount(0, Coin.hbd));
	...
	function displayUnitForCoin(c: typeof Coin.hive | typeof Coin.hbd): string {
		return c.value === Coin.hive.value ? hiveAssetName : hbdAssetName;
	}

	const coin0 = $derived(
		selectedPool && selectedPool.pairSymbols[0].toUpperCase().includes('HBD') ? Coin.hbd : Coin.hive
	);
	const coin1 = $derived(
		selectedPool && selectedPool.pairSymbols[1].toUpperCase().includes('HBD') ? Coin.hbd : Coin.hive
	);

	const maxAmount0 = $derived(
		selectedPool
			? coin0.value === Coin.hbd.value
			...
			if (source === '0') {
				if (amount0Ca.amount === 0) {
					...
					amount1Ca = (await amount0Ca.convertTo(
						coin1,
						Network.lightning
					)) as CoinAmount<typeof Coin.hive | typeof Coin.hbd>;
				}
			} else {
